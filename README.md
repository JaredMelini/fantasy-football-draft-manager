...
- Import user-exported Yahoo Draft Analysis ADP separately from personal rankings and use recent Yahoo market behavior for simulated opponents.
...
The application will not submit draft picks, change rosters, perform unattended Yahoo scraping, collect Yahoo credentials, or redistribute Yahoo data. A user-triggered bookmarklet may export the ADP table already visible in the authorized user's browser into a local CSV; the app never receives the Yahoo session.
...
- Yahoo ADP exports are processed and stored only in the local browser workspace, with their refresh date retained for transparency.
